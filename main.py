...
def input_data() -> dict:
    ...
    input_info['Purchase_amount'] = round(purchase_info['Price']*purchase_info['Number'], 2)
    input_info.update({
...
while game_on:
    ...
    purchase_info = input_data()
    
    payout_dates = []
    
    purchase_info.update(get_bond_data(purchase_info['ISIN']))

    print("Calculating profits part...")
    purchase_info['Holding_period_days'] = (purchase_info['Maturity_date'] - purchase_info['Purchase_date']).days
    purchase_info['Net_income'] = round(purchase_info['Number'] * (1000 + purchase_info['Payout_amount'] * len(payout_dates)) - purchase_info['Purchase_amount'], 2)
